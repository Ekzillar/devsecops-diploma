...
|------------|------------------|--------|
| **Bandit (SAST)** | **Останавливает** | Запускается без `|| true`; при наличии уязвимостей уровня HIGH и выше job `test-and-build` завершается ошибкой, прерывая пайплайн. |
| **OWASP ZAP (DAST)** | Не останавливает | Используется `|| true`, сканирование выполняется без прерывания. При необходимости можно настроить анализ результатов. |
| **Gitleaks** | Не останавливает | Установлен `continue-on-error: true`. Отчёт сохраняется, но pipeline продолжается. |
| **Trivy** | Не останавливает | Параметр `exit-code: '0'` отключает остановку при находках. При желании можно изменить на `'1'`. |
