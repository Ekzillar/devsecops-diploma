...
Статические анализаторы, такие как Bandit, иногда могут выдавать предупреждения о несуществующих уязвимостях. Чтобы не блокировать разработку в таких случаях, в пайплайн добавлен механизм пропуска проверки Bandit с помощью маркера.

Если вы уверены, что срабатывание ложное, добавьте в **заголовок Pull Request** или в **сообщение последнего коммита** фразу `[skip-bandit]`. После этого шаг Bandit будет пропущен, а пайплайн продолжит выполнение и позволит выполнить слияние.

**Пример использования:**
- Заголовок PR: `Refactor authentication module [skip-bandit]`
- Сообщение коммита: `Fix config parsing [skip-bandit]`

**Важно:** используйте этот механизм осознанно и только после проверки, что предупреждение действительно не связано с реальной уязвимостью. Все пропуски фиксируются в истории и могут быть перепроверены при код-ревью.
